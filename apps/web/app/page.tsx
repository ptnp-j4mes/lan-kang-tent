...
import { getActiveBanner, getArticles, getContent, getRecentReviews, popular, topRated } from "@/lib/api";
import { Reveal } from "@/components/reveal";

// offline/API-down fallback for the "เสียงจริงจากสนาม" section.
// keep keys in sync with apps/api/src/lib/site-content.ts (home).
...
const HOME_DEFAULTS = {
  popularEyebrow: "มาแรง",
  popularTitle: "ลานกางเต็นท์ยอดนิยม",
  topRatedEyebrow: "คะแนนสูง",
  topRatedTitle: "ลานกางเต็นท์คะแนนสูง",
  mapEyebrow: "แผนที่ทั่วไทย",
  mapTitle: "ลานกางเต็นท์ทั้งประเทศ ในแผนที่เดียว",
  mapBody: "ซูม เลื่อน คลิกหมุดเต็นท์ เปิดการ์ดดูราคาและรีวิวทันที กรองตามสิ่งที่คุณต้องการ หรือใช้โหมด “ใกล้ฉัน” หาลานใกล้ตัว",
  mapCta: "เปิดแผนที่ลานกางเต็นท์",
  articlesEyebrow: "บทความ & ข่าว",
  articlesTitle: "เรื่องน่ารู้ก่อนออกแคมป์",
  reviewsEyebrow: "เสียงจริงจากสนาม",
  reviewsTitle: "รีวิวล่าสุดจากนักแคมป์",
};

export default async function HomePage() {
  const [banner, articles, c, reviews] = await Promise.all([
    getActiveBanner(),
    getArticles(3),
    getContent("home", HOME_DEFAULTS),
    getRecentReviews(RECENT_REVIEWS),
  ]);
  const pop = popular(8);
  ...
  return (
    <>
      ...
      <SectionRail eyebrow={c.popularEyebrow} title={c.popularTitle} href="/campsites?sort=popular" items={pop} />

      <SectionRail eyebrow={c.topRatedEyebrow} title={c.topRatedTitle} href="/campsites?sort=rating" items={top} />
      ...
      <section className="container py-6">
        ...
        <div className="grain relative overflow-hidden rounded-3xl border border-primary/20 bg-primary text-primary-foreground shadow-lift">
          ...
          <div className="relative grid gap-6 p-8 md:grid-cols-2 md:items-center md:p-12">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-ember">{c.mapEyebrow}</p>
      ...
      {articles.length > 0 && (
        <section className="container py-6">
          <div className="mb-6 flex items-end justify-between gap-4">
            <Reveal>
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-ember">{c.articlesEyebrow}</p>
              <h2 className="mt-1 font-display text-2xl text-primary sm:text-3xl">{c.articlesTitle}</h2>
            </Reveal>
      ...
      <section className="container py-6">
        <div className="mb-6 flex items-end justify-between gap-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-ember">{c.reviewsEyebrow}</p>
            <h2 className="mt-1 font-display text-2xl text-primary sm:text-3xl">{c.reviewsTitle}</h2>
          </div>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {reviews.map((r, i) => (
            <Reveal key={r.id ?? r.name} delay={i * 90}>
            <figure className="relative flex h-full flex-col rounded-2xl border bg-card p-6 shadow-field">
              ...
              <figcaption className="mt-3 border-t pt-3 text-sm">
                ...
                {r.trip && <span className="text-muted-foreground"> · {r.trip}</span>}
